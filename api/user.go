...
func newUserResponse(user db.User) userResponse {
	return userResponse{
		...
		Email:             user.Email,
		PasswordChangetAt: user.PasswordChangetAt,
...
func (s *Server) loginUser(ctx *gin.Context) {
	...
	accessToken, accessTokenPayload, err := s.tokenMaker.CreateToken(user.Username, user.Role, s.config.AccessTokenDuration)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, errorResponse(err))
		return
	}
	...
	session, err := s.store.CreateSession(ctx, db.CreateSessionParams{
		...
		UserAgent:    ctx.Request.UserAgent(), // TODO: fill it latter
